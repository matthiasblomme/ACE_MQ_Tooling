...
<#
.SYNOPSIS
  Library function: switch default ACE environment (machine-level) by running a NEW mqsiprofile.cmd
  under a cleaned baseline env, then persisting the resulting env vars at MACHINE scope.

.DESCRIPTION
  Dot-source this file to load the Persist-AceMqEnv function, then call it with parameters.
  The function is PowerShell 5.1 compatible.
  ...
  Deletions:
    ...
    - Optional: -DeleteExtraVars deletes extra name-matched vars at MACHINE scope
      (those present originally but absent in desired set).
...
function Persist-AceMqEnv {
    <#
    .SYNOPSIS
      Switch default ACE environment (machine-level).
    ...
    .EXAMPLE
      # Simulate only
      Persist-AceMqEnv -AceProfile "C:\Program Files\IBM\ACE\12.0.12.18\server\bin\mqsiprofile.cmd" -OldVersion "12.0.12.17"

    .EXAMPLE
      # Apply to Machine env
      Persist-AceMqEnv -AceProfile "C:\Program Files\IBM\ACE\12.0.12.18\server\bin\mqsiprofile.cmd" -OldVersion "12.0.12.17" -Apply -Normalize
    #>
